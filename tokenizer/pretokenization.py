...
def pretokenize(file_path: str, special_tokens: list[str], num_processes: int, split_token: str = DEFAULT_SPLIT_TOKEN) -> dict[tuple[bytes, ...], int]:
    ...
    with open(file_path, "rb") as f:
        ...
        # Create list of (start, end, file_path, split_token) tuples for each chunk
        chunk_args = [(start, end, file_path, split_token) for start, end in zip(boundaries[:-1], boundaries[1:])]
...
def process_chunk(args: tuple[int, int, str, str]) -> dict[tuple[bytes, ...], int]:
    """
    Processes a chunk of a file to count word occurrences.

    Args:
        args: A tuple containing (start, end, file_path, split_token)
    ...
    """
    start, end, file_path, split_token = args
    with open(file_path, "rb") as f:
        f.seek(start)
